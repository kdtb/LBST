...
class LBSTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        )  # 0 indicates 1st column of the csv file. os.path.join() method in Python join one or more path components intelligently. This method concatenates various path components with exactly one directory separator (‘/’) following each non-empty part except the last path component. If the last path component to be joined is empty then a directory separator (‘/’) is put at the end.
        ...
        if self.transform:
            image = self.transform(
                image
            )  # optional: if we send in trasnforms, it will transform images
